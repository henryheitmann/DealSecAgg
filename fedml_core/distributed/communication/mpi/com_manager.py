...
class MpiCommunicationManager(BaseCommunicationManager):
    def __init__(self, comm, rank, size, node_type="client"):
        ...
        if node_type == "client":
            ...
            self.q_sender, self.q_receiver = self.init_dealer_communication()
        ...
        self.dealer_send_thread = None
        self.dealer_receive_thread = None
        self.dealer_collective_thread = None
    ...
    def stop_receive_message(self):
        ...
        self.__stop_thread(self.dealer_send_thread)
        self.__stop_thread(self.dealer_receive_thread)
        self.__stop_thread(self.dealer_collective_thread)
    ...
    def init_dealer_communication(self):
        dealer_send_queue = queue.Queue(0)
        self.dealer_send_thread = MPISendThread(self.comm, self.rank, self.size, "DealerSendThread", dealer_send_queue)
        self.dealer_send_thread.start()

        dealer_receive_queue = queue.Queue(0)
        self.dealer_receive_thread = MPIReceiveThread(self.comm, self.rank, self.size, "DealerReceiveThread",
                                                      dealer_receive_queue)
        self.dealer_receive_thread.start()

        return dealer_send_queue, dealer_receive_queue
